...
public class Template_UIManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (VD.isActive) //If there is a dialogue active
        {
            ...
            if (!data.pausedAction && !animatingText && data.isPlayer && !useNavigation)
            {
                ...
                {
                    maxPlayerChoices[i].GetComponent<Text>().color = Color.white;
                    if (i == data.commentIndex) maxPlayerChoices[i].GetComponent<Text>().color = Color.yellow;
                }
            }
    ...
    void UpdateUI(VD.NodeData data)
    {
        ...
        NPC_Text.text = "";
        foreach (Button b in maxPlayerChoices) { b.transform.GetComponent<Text>().text = ""; b.transform.GetComponent<Text>().color = Color.white; }
    ...
    public void SetChoices(string[] choices)
    {
        ...
        } else
        {
            ...
            {
                maxPlayerChoices[i].GetComponent<Text>().text = choices[i]; //Assumes first child of button gameobject is text gameobject
    ...
    IEnumerator AnimatePlayerText(string[] choices)
    {
        ...
        for (int c = 0; c < choices.Length; c++)
        {
            ...
            Text txtref = maxPlayerChoices[c].GetComponent<Text>();
            txtref.text = "";
    ...
    void CutTextAnim()
    {
        ...
        if (VD.nodeData.isPlayer)
        {
            ...
            {
                maxPlayerChoices[i].GetComponent<Text>().text = VD.nodeData.comments[i]; //Assumes first child of button gameobject is text gameobject
                maxPlayerChoices[i].gameObject.SetActive(true);
                availableChoices++;
            }
            if (useNavigation)
